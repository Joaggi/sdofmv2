...
def index_nc_files(input_dir: str, start_year: int, end_year: int) -> dict:
    """Scans the directory ONCE and returns a dictionary of counts: {(year, month): count}"""
    lgr_logger.info("Indexing all .nc files on disk. This may take a moment...")
    counts = {}
    ...
    for root, _, files in os.walk(input_dir, followlinks=True):
        for f in files:
            if f.endswith(".nc"):
                ts = parse_filename(f)
                if ts and start_year <= ts.year <= end_year:
                    key = (ts.year, ts.month)
                    counts[key] = counts.get(key, 0) + 1

    lgr_logger.info(f"Finished indexing. Found files for {len(counts)} distinct months.")
    return counts
...
def verify_zarr_store(zarr_path: str, input_dir: str, start_year: int, end_year: int):
    ...
    # Add this right after the initial path checks
    file_counts = index_nc_files(input_dir, start_year, end_year)
    ...
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            ...
            try:
                group_name = f"{year}/{month:02d}"
                ds = xr.open_zarr(zarr_path_p, group=group_name, consolidated=True)
                ...
                if data_array.dims != expected_dims:
                    lgr_logger.error(
                        f"FAIL: Dimensions mismatch in {group_path_str}. Expected {expected_dims}, got {data_array.dims}"
                    )
                    all_checks_passed = False
                ...
                if "time" not in ds.coords:
                    ...
                    if len(ds["time"]) > 1:
                        ...
                        if not np.isclose(time_diff_seconds, expected_12_min_seconds, atol=1):
                            lgr_logger.warning(
                                f"WARNING: Time coordinate frequency might not be 12 minutes in {group_path_str}. First diff: {pd.to_timedelta(time_diff)}"
                            )
                    elif len(ds["time"]) == 1:
                        lgr_logger.debug(
                            f"Only one time step in {group_path_str}, cannot verify frequency."
                        )
                    else:  # len(ds['time']) == 0
                        lgr_logger.warning(
                            f"No time steps found in {group_path_str} despite group existing."
                        )
                ...
                if "channel" not in ds.coords:
                    ...
                    if not all(c in channel_values for c in REQUESTED_CHANNELS):
                        ...
                        lgr_logger.error(
                            f"FAIL: Missing channels in {group_path_str}. Expected subset of {REQUESTED_CHANNELS}, got {channel_values}. Missing: {missing_channels}"
                        )
                        all_checks_passed = False
                    ...
                    if not np.array_equal(channel_values, REQUESTED_CHANNELS):
                        lgr_logger.warning(
                            f"WARNING: Channel order mismatch in {group_path_str}. Expected {REQUESTED_CHANNELS}, got {channel_values}"
                        )

                # 4 & 6. Check shape and match NC file count
                expected_num_nc_files = file_counts.get((year, month), 0)
                expected_shape = (
                    expected_num_nc_files,
                ...
                if data_array.shape != expected_shape:
                    lgr_logger.error(
                        f"FAIL: Shape mismatch in {group_path_str}. Expected {expected_shape} (based on NC files), got {data_array.shape}"
                    )
                    all_checks_passed = False
                else:
                    lgr_logger.info(
                        f"NC file count ({expected_num_nc_files}) perfectly matches Zarr time steps ({actual_num_time_steps}) for {year}/{month:02d}"
                    )

                if data_array.dtype != EXPECTED_DTYPE:
                    lgr_logger.error(
                        f"FAIL: Data type mismatch in {group_path_str}. Expected {EXPECTED_DTYPE}, got {data_array.dtype}"
                    )
                    all_checks_passed = False

                # 5. Check compressor (from encoding attributes)
                if (
                    "compressor" in data_array.encoding
                    and data_array.encoding["compressor"] is not None
                ):
                    ...
                    if hasattr(compressor_obj, "cname"):
                        ...
                        if compressor_name != "lz4":
                            lgr_logger.warning(
                                f"WARNING: Compressor mismatch in {group_path_str}. Expected 'lz4', got '{compressor_name}'"
                            )
                    else:
                        lgr_logger.warning(
                            f"WARNING: Compressor found but cname attribute missing in {group_path_str}. Compressor: {compressor_obj}"
                        )
                else:
                    lgr_logger.warning(
                        f"WARNING: Compressor encoding not found or is None for 'data' in {group_path_str}"
                    )

                # 7. Data Integrity Spot Check (read a small slice and check for NaNs)
                if (
                    actual_num_time_steps > 0 and data_array.shape[1] > 0
                    ...
                    try:
                        ...
                        sample_data = data_array.isel(
                            time=0, channel=0, y=slice(0, 10), x=slice(0, 10)
                        ).compute()
                        if np.all(np.isnan(sample_data.values)):
                            lgr_logger.warning(
                                f"WARNING: Sample data slice is all NaNs in {group_path_str}. This might indicate missing input data for this time/channel or an issue during Zarr writing."
                            )
                        else:
                            lgr_logger.info(
                                f"Sample data slice read successfully from {group_path_str}. Not all NaNs."
                            )
                    except Exception as e:
                        lgr_logger.error(
                            f"FAIL: Error reading sample data from {group_path_str}: {e}"
                        )
                        all_checks_passed = False
                elif actual_num_time_steps == 0:
                    lgr_logger.warning(
                        f"No time steps found in {group_path_str}, skipping NaN check."
                    )

            except FileNotFoundError:
                # This is expected if no .nc files were found for this month/year, and thus no Zarr group was created.
                lgr_logger.debug(
                    f"Zarr group {group_path_str} not found, as expected if no input data existed for this month."
                )
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify Zarr file structure, content, and data integrity."
    )
    parser.add_argument(
        "--zarr_path",
        type=str,
        required=True,
        help="Path to the Zarr store (e.g., /mnt/cedar/surya-bench.zarr)",
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        required=True,
        help="Path to the directory containing the original .nc files.",
    )
    parser.add_argument(
        "--start_year", type=int, required=True, help="Starting year for verification."
    )
