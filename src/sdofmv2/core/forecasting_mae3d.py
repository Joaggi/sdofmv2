...
class TemporalFusion(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_frames: int):
        super().__init__()
        self.fusion = nn.Linear(embed_dim*num_frames, embed_dim)
...
class MaskedAutoencoderViT3D(nn.Module):
    ...
    def __init__(
        ...
        # Pre-calculate patch-level off-limb mask for target 
        num_patches_target = (
            1 * (img_size // patch_size) * (img_size // patch_size)
        )
        patch_off_limb_mask = torch.zeros(num_patches_target, dtype=torch.bool)
        if self.ids_limb_mask is not None:
            patch_off_limb_mask[self.ids_limb_mask] = True
        elif self.limb_mask is not None:
            patch_off_limb_mask = spatial_to_patch_mask(self.limb_mask, patch_size, 1)
        else:
        ...
        # MAE encoder specifics
        self.patch_embed_target = PatchEmbed(
            img_size, patch_size, 1, tubelet_size, in_chans, embed_dim
        )
        num_patches_target = self.patch_embed_target.num_patches

        self.temporal_fusion = TemporalFusion(embed_dim=decoder_embed_dim, num_frames=num_frames)

        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, num_patches_target + 1, decoder_embed_dim), requires_grad=False
        )  # fixed sin-cos embedding
    ...
    def initialize_weights(self):
        ...
        decoder_pos_embed = get_3d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], self.patch_embed_target.grid_size, cls_token=True
        )
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x_frames = x_.reshape(x_.shape[0], self.num_frames, x_.shape[1]//self.num_frames, x_.shape[2])
        x_tf = self.temporal_fusion(x_frames)

        x = torch.cat([x[:, :1, :], x_tf], dim=1)  # append cls token
    ...
    def forward(self, imgs, mask_ratio=0.5):
        ...
        if imgs.shape[2] != 1:
            target =  imgs[:, :, -1:, :, :] # capture the last timestamp 
            input = imgs[:, :, :-1, :, :] # captule all but the last timestamp
        else:
            target =  imgs[:, :, :, :, :] # capture the last timestamp 
            input = imgs[:, :, :, :, :] # captule all but the last timestamp
        ...
        if self.loss_dict.space == "patch":
            target = patchify(target, self.patch_size, self.tubelet_size)
            ...
            if self.loss_dict.exclude_limb:
                limb_mask = self.limb_mask.to(target.device)
                pred_img = pred_img * limb_mask
                target = target * limb_mask

            # Compute pixel-level loss
            if self.loss_dict.type == "vector_aware_loss":
                loss = self.loss_fn(pred_img, target, self.loss_dict.vector_aware_loss.base_loss)
            else:
                loss = self.loss_fn(pred_img, target)
