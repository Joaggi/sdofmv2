...
class MAE(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_frames = num_frames if num_frames == 1 else num_frames - 1 # multitemporal 3D-MAE
    ...
    def training_step(self, batch, batch_idx):
        ...
        if(self.count % 100 == 0):
            ...
            fig, axes = plt.subplots(nrows=2, ncols=12, figsize=(16, 4))
            ...
            for i, col in enumerate(range(12)):
                ...
                axes[0][i].imshow(x[0, i, 0, :, :].detach().cpu(), cmap=cms[i], norm=norm if i>=9 else None)
                axes[1][i].imshow(x_hat[0, i, 0, :, :].detach().cpu(), cmap=cms[i], norm=norm if i>=9 else None)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        num_frames_in = num_frames_in if num_frames_in == 1 else num_frames_in - 1 # multitemporal 3D-MAE
        ...
        # Compute metrics
        step_metrics = compute_metrics_pytorch(target, x_hat_reconstructed, mask_full, self.chan_types)
